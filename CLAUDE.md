...
- The **docs landing page `dist/index.html` (dsh-TUI 上手) is generated** from `pages/cloudflare-pages/index.md` + `_template.html` via `scripts/build-doc.py` (zero-dep stdlib; `npm run build:doc` / `check:doc`). **Edit `index.md`, never `dist/index.html`** — the md is the source of truth, and `test.yml` + `publish.sh --check` both fail on drift (committed HTML must byte-equal the regenerated output). It is *not* an artifact card, so it never enters `src/client/artifacts/`. The other four dist pages are interactive consoles and stay hand-authored.

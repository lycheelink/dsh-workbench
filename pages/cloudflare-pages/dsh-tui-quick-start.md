---
title: dsh-TUI Quick Start
updated: 2026-09-20
footer_source: dsh-workbench 文档 · dsh-TUI Quick Start
footer_applies: "@deepseek-harness-tui/dsh-tui · ccch1mneyyy/dsh-TUI"
footer_note: 启用“完全访问”将放开文件系统与命令执行权限，且不再有审批拦截，请自行评估风险后使用。
---

<!--
dsh-workbench 文档源 · 由 scripts/build-doc.py 渲染成 dist/dsh-tui-quick-start.html（零依赖 python3）。
改完本文件后运行：python3 scripts/build-doc.py 重新生成；--check 校验与已提交 HTML 一致。
本注释下方的「标准 markdown / 指令块」词表为全部生成页共用（首页 index.md、dsh-quick-start.md 同款）。
...
- [返回首页](index.html) —— dsh-workbench 站点头页：插件说明、在线工具入口与全部文档索引。
- [DSH 安装使用手册](dsh-quick-start.html) —— 姊妹篇：Web 版宿主本体的安装、插件、技能与排错手册。
